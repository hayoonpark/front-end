...
class Router{
    ...
    addRoute(url, callback){
        ...
        // chaining 사용을 위한 인스턴스 리턴 e.g. Router.addRoute().addRoute()
        return this;
    }
...
class TodoList{
    ...
    initStorage(storage){
        this.storage = storage //class변수로 넣어주기
    }
    ...
    assignElement(){
        ...
        this.radiaAreaEl = this.inputContainerEl.querySelector("#radio-area")
        this.filterRadioBtnEls = this.radiaAreaEl.querySelectorAll('input[name="filter"]')
    }
    ...
    onClickRadioBtn(event){
        ...
        this.filterTodo(value) //Router에서 구현하고있기에 지움
    ...
    onClickTodoList(event){
        const {target} = event
        //button태그 안에 svg를 눌렀을때도 동작하게 하기위함
        const btn = target.closest("button")
        if(!btn) return
        if(btn.matches("#delete-btn")){ this.deleteTodo(target)}
        else if(btn.matches("#edit-btn")){this.editTodo(target)}
        else if(btn.matches("#save-btn")){this.saveTodo(target)}
        else if(btn.matches("#complete-btn")){this.completeTodo(target)}
    }
    ...
    deleteTodo(target){
        ...
        console.log(todoDiv.dataset.id)
